...
const ProductsCarrefour = () => {
  ...
  // Estado para controlar la paginación y la búsqueda
  const [pagina, setPagina] = useState(0);
  const [hayMasResultados, setHayMasResultados] = useState(true);
  const [busquedaActual] = useState("leche");

  // Estado para el filtro de contenido seleccionado
  const [contenidoFiltro, setContenidoFiltro] = useState("");

  // Función principal para obtener productos (primera carga o paginación)
  const obtenerProductosCarrefour = async (pageIndex = 0) => {
    ...
    // Calculamos el rango de ítems según la página (0-49, 50-99, 100-149...)
    const from = pageIndex * PRODUCTOS_POR_PAGINA;
    const to = from + PRODUCTOS_POR_PAGINA - 1;

    try {
      const res = await fetch(
        `/api-carrefour/${busquedaActual}?_from=${from}&_to=${to}`,
      );
      const data = await res.json();

      if (Array.isArray(data) && data.length > 0) {
        const productosLimpios = mapearProductoCarrefour(data).map((p) => ({
          ...p,
          logoTienda: SUPERMARKET_LOGOS.carrefour,
        }));

        // Si es la primera página, reemplazamos el array. Si es paginación, anexamos.
        setProductos((prev) =>
          esPrimeraCarga ? productosLimpios : [...prev, ...productosLimpios],
        );

        // Si vinieron menos de 50 ítems, significa que alcanzamos el final del catálogo
  ...
  const manejarCargarMas = () => {
    ...
    obtenerProductosCarrefour(SiguientePagina);
  };

  // 1. Extraer lista única de contenidos disponibles de TODOS los productos acumulados
  const opcionesContenido = useMemo(() => {
    const todosLosContenidos = productos
      .map((p) => p.contenido)
      .filter((c) => c && c.trim() !== "");
    return [...new Set(todosLosContenidos)];
  }, [productos]);

  // 2. Filtrar los productos según la opción seleccionada
  ...
  return (
    <div className="p-6 bg-slate-900 min-h-screen text-white">
      ...
      <div className="max-w-xs mx-auto mb-6 flex flex-col items-center gap-2">
        ...
        >
          <option value="">Todos los contenidos ({productos.length})</option>
          ...
          {hayMasResultados && (
            <div className="mt-10 flex justify-center">
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-2.5 px-6 rounded-xl shadow-lg transition-all border border-emerald-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
              >
