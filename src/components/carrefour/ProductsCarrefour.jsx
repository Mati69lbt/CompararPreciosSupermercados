...
const ProductsCarrefour = () => {
  ...
  const cargarPaginasIniciales = async () => {
    ...
    try {
      ...
      const peticiones = paginasACargar.map((pageIndex) => {
        ...
        return fetch(`/api-carrefour/${busquedaActual}?_from=${from}&_to=${to}`)
          .then((res) => res.json())
          .catch(() => []);
      });
  ...
  // 2. Extraer lista única de CONTENIDOS/MEDIDAS disponibles
  ...
  // 3. Filtrar por Marca/Medida y ORDENAR DE MENOR A MAYOR PRECIO
  ...
  return (
    <div className="p-6 bg-slate-900 min-h-screen text-white">
      <h2 className="text-2xl font-bold mb-6 text-center">
        Resultados Carrefour ({productos.length} productos cargados)
      </h2>
