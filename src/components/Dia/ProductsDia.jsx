...
const ProductsDia = () => {
  ...
  const obtenerProductosDia = async (pageIndex = 0) => {
    ...
    try {
      // Usamos 'ft' para el término de búsqueda y los rangos de paginación
      const res = await fetch(
        `/api-dia?ft=${busquedaActual}&_from=${from}&_to=${to}`,
      );
      const data = await res.json();

      if (Array.isArray(data) && data.length > 0) {
        const productosLimpios = mapearProductoDia(data).map((p) => ({
          ...p,
  ...
  useEffect(() => {
    obtenerProductosDia(0);
  }, [busquedaActual]);

  const manejarCargarMas = () => {
    const siguientePagina = pagina + 1;
    setPagina(siguientePagina);
    obtenerProductosDia(siguientePagina);
  };
          ...
          {hayMasResultados && (
            <div className="mt-10 flex justify-center">
              <button
                ...
                className="bg-red-600 hover:bg-red-500 text-white font-semibold py-2.5 px-6 rounded-xl shadow-lg transition-all border border-red-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
              >
...
// PrecioPorUnd: ['2290.00'] , UnidaddeMedida: ['1 LT'], para diferenciar por cantidadm, brand,
