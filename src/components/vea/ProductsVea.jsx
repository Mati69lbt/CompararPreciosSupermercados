...
const ProductsVea = () => {
  ...
  const obtenerProductosVea = async (pageIndex = 0) => {
    ...
    try {
      const res = await fetch(
        `/api-vea?ft=${busquedaActual}&_from=${from}&_to=${to}`,
      );
      const data = await res.json();
      console.log("data", data);

      if (Array.isArray(data) && data.length > 0) {
        const productosLimpios = mapearProductoVea(data).map((p) => ({
          ...p,
  ...
  useEffect(() => {
    obtenerProductosVea(0);
  }, [busquedaActual]);

  const manejarCargarMas = () => {
    const siguientePagina = pagina + 1;
    setPagina(siguientePagina);
    obtenerProductosVea(siguientePagina);
  };
